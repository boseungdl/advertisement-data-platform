...
[병렬] Airflow on Docker
   ├─ ads_silver_merge_hourly      (매시 정각, 1시간 주기)
...
3. **컴팩션 vs streaming MERGE 동시성 안전 (Snapshot Isolation)** — 작은 파일 청소 잡과 1시간 주기 MERGE가 같은 파티션을 동시에 건드려도 OCC가 충돌 감지·재시도. Plain Parquet+Hive는 락 직접 관리.

4. **micro-batch + batch의 작은 파일 누적 자동화** — Bronze streaming(5분)과 Silver/Gold 1시간 배치가 만드는 작은 parquet들을 `rewrite_data_files` procedure로 컴팩션. **운영자 개입 없이** 평균 파일 크기 유지.
...
| Silver MERGE lag P95 | 1시간 30분 초과 (= 1시간 주기 + 30분 여유) | 배치 잡 정체 | 1·2 | #7 |
...
- **조치**:
  1. **시간대 분리** — 매니지먼트 03:00~05:00 KST 사이에만 (`ads_silver_merge_hourly`는 그 시간 자동 일시 정지하도록 Airflow `schedule_interval` 조건 추가)
