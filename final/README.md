...
| **멱등성** | 같은 윈도우 두 번 처리 → 결과 **bit-identical** | 새벽 호출 시 안전 재실행, 3개월 백필 안전 — 운영 신뢰의 안전벨트 |
...
이 아키텍처는 광고 도메인의 본질 — **클릭 후 며칠 뒤에 도착하는 전환 데이터(late-arriving conversion)** 를 어떻게 안전하게 처리할까 — 에서 출발했습니다.

이 한 가지 고민에서 세 가지 결정이 나왔고, 각 결정이 컴포넌트 선택의 근거입니다:

| 결정 | 근거 |
|---|---|
| **Bronze는 plain Parquet, Iceberg 아님** | raw는 백필 안전선이라 절대 손상되면 안 됨. **append-only면 충분** — MERGE/스냅샷 가치 없음. Iceberg 관리 부담만 늘 뿐. |
| **Silver는 Iceberg(오픈테이블포맷)** | 며칠 뒤 도착하는 전환을 어제 표에 안전하게 갱신하려면 **MERGE 멱등성이 필수**. plain Parquet으로는 불가능 — 이게 Iceberg 채택의 가장 큰 근거. |
| **Gold는 두 종류로 분리** | 같은 정제 데이터지만 **운영자가 보는 헬스 메트릭**과 **캠페인 매니저가 보는 비즈니스 KPI**는 갱신 주기·집계 단위·소비자 페르소나가 모두 다름. 한 테이블에 섞으면 어느 쪽도 만족 못함. |
