...
| KPI | 측정 | 의미 (왜 이걸 보나) |
|---|---|---|
| **처리량** | 일 100만 이벤트 처리 (10x 성장 시 일 1,000만) | 광고 도메인 표준 규모 — 강의 가정과 일치 |
| **신선도** | late-arriving conversion **P95 1시간 안에** Gold 반영 | 어제 만든 캠페인 리포트가 오늘 늦게 도착한 전환 데이터로 자동 갱신 — 광고 도메인 본질 |
| **멱등성** | 같은 윈도우 두 번 처리 → 결과 **bit-identical** *(설계: MERGE 키 + `ingested_at` 가드)* | **운영의 안전벨트** — 새벽 OOM 자동 복구, 3개월 백필 안전, **사람 개입 0건** *(§5 알람 임계로 측정)* |
...
"매일 5분 헬스체크"가 막연한 약속이 아니라 **측정 가능한 임계**로 단단해집니다. 위반 시 자동 알람(Slack/이메일):

| 메트릭 | 알람 임계 | 위반이 의미하는 것 | 대응 쿼리 |
|---|---|---|---|
| Bronze 신선도 | 최신 `event_time`이 `now() - 10분` 초과 | streaming 2 배치 이상 지연 (5분 micro-batch가 두 번 멈춤) | #1 |
| Silver MERGE lag P95 | 15분 초과 | 배치 잡 정체 — Spark 메모리 또는 S3 throttling 의심 | #7 |
| 멱등성 위반 | `event_id` 중복 1건 이상 | 데이터 깨짐 — 즉시 백필 + 원인 추적 | #6 |
| 스냅샷 누적 수 | 1,000개 초과 | Expire DAG가 작동 안 함 — 저장 비용 폭증 위험 | #4 |
| 작은 파일 비율 | 파티션당 평균 < 128MB | 컴팩션이 못 따라옴 — 쿼리 성능 저하 | #3 |

→ 새벽 호출이 무서운 게 아니라, **무엇을 봐야 하는지 명확하지 않은 게 무서운** 것. 임계가 박혀 있으면 운영자가 자기 페이스로 처리.
