...
- **트래픽 패턴**: 피크 12~18시 + 21~23시 평소의 2~3배, 주말 1.5배 (광고 도메인 시즌 스파이크 — 블프/광군제 3~5배 등 — 은 **§7**에서 깊이)
- **성장 가정 — 이 프로젝트 설계의 핵심 근거**:
  - 6개월 내 10x → 일 1,000만 이벤트
  - **1~2년 내 100x → 일 1억(일 ~100GB)** ← 모든 아키텍처·스키마 결정이 이 가정을 통과해야 함. **§7 스케일 아웃 시나리오의 출발점**.
