...
### 0-1. 원본 — Criteo Attribution Modeling Dataset

[Diemert et al., 2017] — 30일간 실 광고 운영 트래픽의 click/impression/conversion 로그.

| 파일 | 용도 | 행 수 | 컬럼 수 |
|---|---|---|---|
| `criteo_attribution_dataset.tsv.gz` | 풀스케일 광고 이벤트 | 16,468,027 | 22 |

22 컬럼 = `timestamp`, `uid`, `campaign`, `conversion`, `conversion_timestamp`, `attribution`, `click`, `click_pos`, `click_nb`, `cost`, `cpo`, `time_since_last_click`, `cat1~cat9`. 1 row = 1 click 또는 1 impression.

데이터 특성:
- **약 700개 캠페인 × 30일** ≈ 일 547K events
- **CTR ~3%, CVR(of clicks) ~5%** — 광고 도메인 평균 수준
- **conversion_timestamp − timestamp 분포**: 즉시 전환부터 **5일 이상 지연**까지 long-tail (= late-arriving conversion 도메인의 본질)
- 라이센스: CC BY-NC-SA 4.0 (비상업 한정 — 발표·포트폴리오 OK)

[NASA-style 학술 인용](https://research.criteo.com/) — 광고 attribution 학술 표준 데이터셋. `data/criteo_attribution_dataset.tsv.gz`로 받아 `final/infra/data/`에 둠 (`.gitignore` 대상).

### 0-2. 시뮬레이션 — `prepare_criteo_data.py` (timestamp rebasing)

원본은 정적 TSV로 timestamp가 **상대 초**(0 ~ 2,592,000). 운영 lakehouse 검증을 위해 절대 시각으로 합성한 뒤 Kafka로 발행.

**event_time 합성 공식**:
```
event_time = base_datetime + 원본_relative_seconds
```
- `--base-datetime 2026-04-01T00:00:00` (UTC) — 시뮬 기준 시각, 동일 base 면 데이터 재현 가능
- 원본의 30일 span이 그대로 이어져 `2026-04-01 ~ 2026-04-30` 절대시각으로 매핑

인자 표:
| 인자 | 역할 | 기본값 |
|---|---|---|
| `--input` | Criteo TSV.GZ 경로 | (필수) |
| `--output` | CSV 출력 디렉토리 | `./data` |
| `--sample` | reservoir sampling 행 수 | 1,000,000 |
| `--base-datetime` | event_time rebasing 기준 시각 | `2026-04-01T00:00:00` |
| `--keep-cats` | cat1~9 카테고리 컬럼 유지 | off |
| `--seed` | 샘플링 시드 | 42 |

산출 3종:
- `ad_events.csv` — 메인 (default 100만건, ~150MB)
- `ad_events_sample.csv` — 처음 1000건 (빠른 스모크)
- `ad_events_batch2.csv` — 마지막 20% (백필 시뮬)

**왜 시간 컬럼을 두 개로 분리했나** — `event_time`은 광고가 일어난 시각(시뮬 도메인), `ingested_at`은 Bronze 적재 시각(메타 시간). Iceberg time-travel의 두 시간 축(`AS OF snapshot` vs `WHERE event_time BETWEEN`)을 모두 시연하기 위함. 멱등성 보장도 `ingested_at` 기준.

대안 시나리오:
| 의도 | 인자 |
|---|---|
| 빠른 데모 | `--sample 100000` |
| 풀스케일 (16.4M) | `--sample 16400000` |
| 90일 백필 시연 | `--base-datetime 2026-02-01T00:00:00` |
| 결정성 검증 | 같은 `--seed` + 같은 `--base-datetime` → bit-identical 출력 |

---

## 1. 도메인 정의 + 핵심 KPI 3개

**도메인**: 광고대행사·광고주의 **멀티캠페인 fleet 성과 추적** + **어트리뷰션 모델 비교** + **late-arriving conversion 모니터링**.

**핵심 페르소나** (상세는 [docs/PRD.md](../docs/PRD.md))
- **P1 캠페인 매니저(민혁, 28세, 광고대행사 운영팀)** — 매일 700+ 캠페인의 어제·오늘 ROI/CTR/CVR을 추적해 광고비 배분 결정. 어제 "효과 없다" 판단한 캠페인이 5일 뒤 전환 들어오며 결과 뒤집히는 경험으로 어트리뷰션·신선도에 민감.
- **P2 파이프라인 운영자(지수, 31세, 데이터플랫폼팀)** — 매일 출근 직후 5분 헬스체크. 새벽 OOM 호출 안 받게 멱등성·체크포인트가 단단해야 함.
...
- **변환 규칙**:
  - JSON payload → 22 컬럼 분해
  - timestamp UTC 표준화 (ms 단위)
  - **late-arriving conversion 갱신** — 같은 click의 `event_id`에 conversion 정보 MERGE
